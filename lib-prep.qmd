...
```{r}
tibble(reagent = c("NF water", "Sample"), Volume = c("9 uL", "1 uL")) |> 
  gt(rowname_col = "reagent") |> 
...
For samples with low region molarity (As dertermined by TapeStation), you might want to use 1:100 dilutions. For all others, use 1:1000.
...
3. Label them 1-5. Using a P20 pipette, take $4 \mu L$ __E. coli DH10B Control Library__ and add it to tube 1. Mix by pipetting 10 times. Using the same tip, take $4 \mu L$ from tube 1 and add it to tube 2. Mix by pipetting 10 times. Repeat until you have mixed tubes 1-5.

3. Determine the number wells you will need. Each sample is run in triplicate, you need a triplicate of water, and it helps to have 5 of each control. So,
$$
(n_{sample} \times 3) + (n_{H_2O} \times 3) + (n_{control} \times 5) + extra
$$

$$
(n_{sample} \times 3) + (1 \times 3) + (5\times 5) + 10
$$

4. Make the mastermix. For $n$ wells, do $vol_{reagent}(n+1)$ for the following:
```{r}
