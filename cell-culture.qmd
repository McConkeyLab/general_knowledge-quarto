...
```{r}
#| echo: false
tibble(`Flask Size` = c("T25", "T75", "T175"), `Medium Vol` = c("5mL", "10mL", "25mL"), `Trypsin Vol` = c(".5mL", "1.3mL", "3mL")) |> 
...
2.  Wipe down the vial with ethanol, then open.
...
6.  Look for a pellet and note its size
7.  Aspirate the medium from the pellet and resuspend in medium
    -   If the pellet was quite large, resuspend in 10mL medium and transfer to a T75.
    -   If the pellet was smaller, resuspend in 5mL medium and transfer to a T25.
...
11. Prepare a new flask by writing the cell line, passage number, ratio of split (ie 1:10, see below) initials, and date
...
1.  Follow the [cell-splitting protocol](#split-cells) up and through transfering the cell suspension into a conical tube.
